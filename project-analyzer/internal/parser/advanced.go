...
func (p *FileParser) DetectProjectType(folderAnalysis signals.FolderAnalysis, codeSignals signals.CodeSignals) signals.ProjectType {
	...
	// Check for nested services inside a services/ folder
	servicesPath := filepath.Join(p.repoPath, "services")
	if entries, err := os.ReadDir(servicesPath); err == nil {
		for _, entry := range entries {
			if entry.IsDir() {
				entryName := strings.ToLower(entry.Name())
				// Count directories that look like services (exclude common non-service folders)
				if !strings.HasPrefix(entryName, ".") &&
					entryName != "node_modules" &&
					entryName != "dist" &&
					entryName != "build" {
					// Check if it has package.json or go.mod or other service indicators
					servicePath := filepath.Join(servicesPath, entry.Name())
					hasPackageJSON := fileExists(filepath.Join(servicePath, "package.json"))
					hasGoMod := fileExists(filepath.Join(servicePath, "go.mod"))
					hasDockerfile := fileExists(filepath.Join(servicePath, "Dockerfile"))

					if hasPackageJSON || hasGoMod || hasDockerfile {
						serviceCount++
					}
				}
			}
	...
	for _, folder := range topFolders {
		...
		// Count service-like folders at top level (but exclude the generic "services" folder itself)
		if (strings.Contains(folderLower, "service") || strings.Contains(folderLower, "svc")) &&
			folderLower != "services" {
			serviceCount++
		}
	}
