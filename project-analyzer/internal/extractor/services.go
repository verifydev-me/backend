...
func (e *InfraExtractor) extractDeepServiceSignals() {
	// Track services found in nested folders
	var nestedServices []string
	...
	for _, folder := range serviceFolders {
		...
		for _, entry := range entries {
			...
			// Check if this is actually a service (has package.json, go.mod, etc.)
			servicePath := filepath.Join(folderPath, name)
			isService := fileExists(filepath.Join(servicePath, "package.json")) ||
				fileExists(filepath.Join(servicePath, "go.mod")) ||
				fileExists(filepath.Join(servicePath, "requirements.txt")) ||
				fileExists(filepath.Join(servicePath, "Dockerfile"))

			if isService && !isFrontendFolder(name) {
				nestedServices = append(nestedServices, name)
			}
	...
	// Update service count and names if nested services were found
	if len(nestedServices) > 0 {
		// Update service count (max of existing count or nested count)
		if len(nestedServices) > e.signals.ServiceCount {
			e.signals.ServiceCount = len(nestedServices)
			e.signals.ServiceNames = nestedServices
		}

		// Add microservices signal if we have 2+ services
		if len(nestedServices) >= 2 {
			e.signals.AddSignal(signals.SignalMultipleServices, 0.95, nestedServices, "nested_services")
			e.signals.AddSignal(signals.SignalServiceIsolation, 0.9, nestedServices, "independent_nested_services")
		}
	}
}
