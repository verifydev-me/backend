...
router.get('/my-interviews', async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const interviews = isRecruiter
      ...
      : await interviewService.getUserInterviews(req.user!.userId, status as any);
...
router.post('/:id/reschedule', async (req: AuthenticatedRequest, res, next) => {
  try {
...
router.post('/:id/no-show', async (req: AuthenticatedRequest, res, next) => {
  try {
