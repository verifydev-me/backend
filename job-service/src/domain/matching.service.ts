...
export class MatchingService {
  ...
  private matchExperience(coreCount: number, level: string, minExp?: number): number {
  ...
  // Match location
  private matchLocation(candidate: any, job: any): number {
  ...
  // Match availability
  private matchAvailability(candidateAvailable?: Date, jobDate?: Date): number {
  ...
  async findMatchingJobs(candidateId: string, limit = 20) {
  ...
  async findMatchingCandidates(jobId: string, filters?: CandidateSearchFilters) {
  ...
  async getSuggestedCandidates(jobId: string, recruiterId: string, limit = 10) {
