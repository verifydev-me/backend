...
router.get('/conversation/:otherUserId', async (req, res, next) => {
    try {
        ...
        const messages = await messageService.getConversation(req.user.userId, req.params.otherUserId, jobId);
        res.json({ success: true, data: messages });
    }
    catch (error) {
        next(error);
...
router.get('/application/:applicationId', async (req, res, next) => {
    try {
        const messages = await messageService.getApplicationMessages(req.params.applicationId, req.user.userId);
        res.json({ success: true, data: messages });
    }
    catch (error) {
        next(error);
